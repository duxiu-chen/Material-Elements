...
import kotlinx.android.synthetic.main.activity_progress_dots_bounds.*
import java.util.*

class ProgressDotsBoundsActivity: AppCompatActivity() {
    private var data = LinkedList<ProgressBeans>()
    private lateinit var adapter: ProgressAdapter
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_progress_dots_bounds)
        initView()
        initListener()
    }
    ...
    fun startAnim(){
        ...
        handler.postDelayed(Runnable {
            ...
            anim.addUpdateListener {
                ...
                if (x == 0f){
                    recyclerView.layoutAnimation = AnimationUtils.loadLayoutAnimation(this@ProgressDotsBoundsActivity, R.anim.layout_anim_fade_in_list_item)
                    recyclerView.layoutManager = LinearLayoutManager(this@ProgressDotsBoundsActivity)
                    initData()
